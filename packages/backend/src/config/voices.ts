...
/**
 * 预置音色列表（MiMo 9 + edge 10 中文子集）
 * 被 voices 路由和 tts 路由共享
 */
export const PRESET_VOICES: VoiceEntry[] = [
  // === MiMo 预置 9 ===
...
/**
 * 根据 voiceId 查找预置音色的 nativeId
 * @param voiceId 内部音色 ID，如 "mimo-冰糖"
 * @returns nativeId，如 "冰糖"；找不到返回 undefined
 */
export function findNativeVoiceId(voiceId: string): string | undefined {
  const voice = PRESET_VOICES.find(v => v.id === voiceId);
  if (voice && voice.kind === 'preset') {
    return voice.nativeId;
  }
  return undefined;
}
